#include "stdafx.h"
#include "..\public\Point_Trail.h"

CPoint_Trail::CPoint_Trail(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
	: CGameObject(pDevice, pDevice_Context)
{
}

CPoint_Trail::CPoint_Trail(const CPoint_Trail & rhs)
	: CGameObject(rhs)
{
}

HRESULT CPoint_Trail::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CPoint_Trail::NativeConstruct(void * pArg)
{
...
_int CPoint_Trail::Tick(_float TimeDelta)
{
...
_int CPoint_Trail::Late_Tick(_float TimeDelta)
{
...
void CPoint_Trail::Move_Spread(_float TimeDelta)
{
	m_PointDesc.fLifeTime -= TimeDelta;
...
VTXMATRIX * CPoint_Trail::Get_InstanceBuffer()
{
	return m_pBufferInstanceCom->Get_InstanceBuffer();
}

void CPoint_Trail::Set_InstanceBuffer(VTXMATRIX* pInstanceBuffer)
{
	m_pBufferInstanceCom->Set_InstanceBuffer(pInstanceBuffer);
}

HRESULT CPoint_Trail::Ready_Component(void * pArg)
{
	HRESULT hr = S_OK;

	memcpy(&m_PointDesc, pArg, sizeof(POINT_TRAIL_DESC));
	...
	if (hr != S_OK)
		MSG_BOX("CPoint_Trail::Ready_Component Failed");

	return hr;
}

void CPoint_Trail::Set_Pos(_fvector vPos)
{
	m_pMovementCom->Set_State(EState::Position, vPos);
}

void CPoint_Trail::SetUp_IndexDir(_int iRandNum_Max)
{
...
CPoint_Trail * CPoint_Trail::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
{
	CPoint_Trail*		pInstance = new CPoint_Trail(pDevice, pDevice_Context);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Creating Instance (CPoint_Trail) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CPoint_Trail * CPoint_Trail::Clone_GameObject(void * pArg)
{
	CPoint_Trail*		pInstance = new CPoint_Trail(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned Instance (CPoint_Trail) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CPoint_Trail::Free()
{
	__super::Free();

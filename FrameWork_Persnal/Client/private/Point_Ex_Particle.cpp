#include "stdafx.h"
#include "..\public\Point_Ex_Particle.h"
#include "Point_Ex_Healing.h"

CPoint_Ex_Particle::CPoint_Ex_Particle(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
	: CPoint_Ex_Trail(pDevice, pDevice_Context)
{
}

CPoint_Ex_Particle::CPoint_Ex_Particle(const CPoint_Ex_Particle & rhs)
	: CPoint_Ex_Trail(rhs)
{
}

HRESULT CPoint_Ex_Particle::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CPoint_Ex_Particle::NativeConstruct(void * pArg)
{
...
_int CPoint_Ex_Particle::Tick(_float TimeDelta)
{
...
_int CPoint_Ex_Particle::Late_Tick(_float TimeDelta)
{
	if (0 >= m_PointDesc.fLifeTime)
		return 0;
...
HRESULT CPoint_Ex_Particle::Render()
{
...
CPoint_Ex_Particle * CPoint_Ex_Particle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
{
	CPoint_Ex_Particle*		pInstance = new CPoint_Ex_Particle(pDevice, pDevice_Context);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Creating Instance (CPoint_Ex_Particle) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CPoint_Ex_Particle * CPoint_Ex_Particle::Clone_GameObject(void * pArg)
{
	CPoint_Ex_Particle*		pInstance = new CPoint_Ex_Particle(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned Instance (CPoint_Ex_Particle) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CPoint_Ex_Particle::Free()
{
	__super::Free();
}

#include "stdafx.h"
#include "..\public\MyButton_NoText.h"
#include "Cursor_Manager.h"

CMyButton_NoText::CMyButton_NoText(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
	: CUI_2D(pDevice, pDevice_Context)
{
}

CMyButton_NoText::CMyButton_NoText(const CMyButton_NoText & rhs)
	: CUI_2D(rhs)
{
}

HRESULT CMyButton_NoText::NativeConstruct_Prototype(void * pArg)
{
	return S_OK;
}

HRESULT CMyButton_NoText::NativeConstruct(void * pArg)
{
	UI2D_DESC* pData = new UI2D_DESC;

	memcpy(pData, pArg, sizeof(UI2D_DESC));

	__super::SetUp_Default_Component(pData);

	Safe_Delete(pData);

	return S_OK;
}

_int CMyButton_NoText::Tick(_float TimeDelta)
{
	Button_Picking_Check();
	return _int();
}

_int CMyButton_NoText::Late_Tick(_float TimeDelta)
{
	return 0; //m_pRendererCom->Add_GameObjectToRenderer(ERenderGroup::FrameUI, this);
}

HRESULT CMyButton_NoText::Render(_uint iTextureIndex, _uint ShaderPass)
{
	if (nullptr != m_pTextureCom)
	{
		m_pBufferRectCom->Set_ShaderResourceView("g_DiffuseTexture", m_pTextureCom->Get_ShaderResourceView(iTextureIndex));
		m_pBufferRectCom->Set_Variable("WorldMatrix", &XMMatrixTranspose(m_pMovementCom->Get_WorldMatrix()), sizeof(_matrix));
		//m_pBufferRectCom->Set_Variable("ViewMatrix", &XMMatrixTranspose(GET_INDENTITY_MATRIX), sizeof(_matrix));
		//m_pBufferRectCom->Set_Variable("ProjMatrix", &XMMatrixTranspose(GET_ORTHO_SPACE), sizeof(_matrix));

		m_pBufferRectCom->Render(ShaderPass);

// 		D3D11_SHADER_RESOURCE_VIEW_DESC Data;
// 		m_pTextureCom->Get_ShaderResourceView(iTextureIndex)->GetDesc(&Data);
// 		Data.Texture2DMS.UnusedField_NothingToDefine
	}

	return S_OK;
}

_bool CMyButton_NoText::Get_IsClick()
{	
	m_IsPick = false;

	if (CGameInstance::GetInstance()->Get_InputDev()->Get_MouseClick_Down(MOUSEKEYSTATE::LB)/*GetAsyncKeyState(VK_LBUTTON) & 0x8000*/)
	{
		POINT pt = GET_CURSOR_XY;
		GetCursorPos(&pt);
		ScreenToClient(g_hWnd, &pt);

		_float4 vMyPos;
		XMStoreFloat4(&vMyPos, m_pMovementCom->Get_State(EState::Position));
		_float2 vScale = { m_pMovementCom->Get_Scale(EState::Right), m_pMovementCom->Get_Scale(EState::Up) };
		vMyPos.x = (g_iWinCX * 0.5f) + vMyPos.x;
		vMyPos.y = (g_iWinCY * 0.5f) - vMyPos.y;

		m_tPickingBox.left = (LONG)(vMyPos.x - (vScale.x * 0.5f));
		m_tPickingBox.top = (LONG)(vMyPos.y - (vScale.y * 0.5f));
		m_tPickingBox.right = (LONG)(vMyPos.x + (vScale.x * 0.5f));
		m_tPickingBox.bottom = (LONG)(vMyPos.y + (vScale.y * 0.5f));


		if (PtInRect(&m_tPickingBox, pt))
			m_IsPick = true;

		if (m_IsPick == true)
			return true;
	}
	return false;
}

_bool CMyButton_NoText::Get_IsPick()
{
	if (m_IsPick == true)
		return true;
	
	return false;
}

HRESULT CMyButton_NoText::Ready_Component()
{
	return S_OK;
}

void CMyButton_NoText::Button_Picking_Check()
{
	POINT pt = GET_CURSOR_XY;
	GetCursorPos(&pt);
	ScreenToClient(g_hWnd, &pt);

	_float4 vMyPos;
	XMStoreFloat4(&vMyPos, m_pMovementCom->Get_State(EState::Position));
	_float2 vScale = { m_pMovementCom->Get_Scale(EState::Right), m_pMovementCom->Get_Scale(EState::Up) };
	vMyPos.x = (g_iWinCX * 0.5f) + vMyPos.x;
	vMyPos.y = (g_iWinCY * 0.5f) - vMyPos.y;

	m_tPickingBox.left = (LONG)(vMyPos.x - (vScale.x * 0.5f));
	m_tPickingBox.top = (LONG)(vMyPos.y - (vScale.y * 0.5f));
	m_tPickingBox.right = (LONG)(vMyPos.x + (vScale.x * 0.5f));
	m_tPickingBox.bottom = (LONG)(vMyPos.y + (vScale.y * 0.5f));


	if (PtInRect(&m_tPickingBox, pt))
		m_IsPick = true;
	else
		m_IsPick = false;
}

CMyButton_NoText * CMyButton_NoText::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context, void * pArg)
{
	CMyButton_NoText* pInstance = new CMyButton_NoText(pDevice, pDevice_Context);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Creating Instance (CMyButton_NoText) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMyButton_NoText::Clone_GameObject(void * pArg)
{
	return nullptr;
}

void CMyButton_NoText::Free()
{
	__super::Free();

}

#include "stdafx.h"
#include "..\public\Rect_Spread.h"

CRect_Spread::CRect_Spread(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
	: CGameObject(pDevice, pDevice_Context)
{
}

CRect_Spread::CRect_Spread(const CRect_Spread & rhs)
	: CGameObject(rhs)
{
}

HRESULT CRect_Spread::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CRect_Spread::NativeConstruct(void * pArg)
{
...
_int CRect_Spread::Tick(_float TimeDelta)
{
...
_int CRect_Spread::Late_Tick(_float TimeDelta)
{
...
HRESULT CRect_Spread::Ready_Component(void * pArg)
{
	HRESULT hr = S_OK;

	memcpy(&m_PointDesc, pArg, sizeof(RECT_SPREAD_DESC));
	...
	if (hr != S_OK)
		MSG_BOX("CRect_Spread::Ready_Component Failed");

	return hr;
}

void CRect_Spread::Set_Pos(_fvector vPos)
{
	m_pMovementCom->Set_State(EState::Position, vPos);
}

void CRect_Spread::SetUp_IndexDir(_int iInstanceIndex)
{
...
void CRect_Spread::Check_Instance(_float TimeDelta)
{
	// 0번을 주위로 퍼져나감
...
CRect_Spread * CRect_Spread::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
{
	CRect_Spread*		pInstance = new CRect_Spread(pDevice, pDevice_Context);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Creating Instance (CRect_Spread) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CRect_Spread * CRect_Spread::Clone_GameObject(void * pArg)
{
	CRect_Spread*		pInstance = new CRect_Spread(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned Instance (CRect_Spread) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CRect_Spread::Free()
{
	__super::Free();

#include "..\public\VIBuffer_PointInstance_Extend.h"

CVIBuffer_PointInstance_Extend::CVIBuffer_PointInstance_Extend(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
	: CVIBuffer(pDevice, pDevice_Context)
{

}

CVIBuffer_PointInstance_Extend::CVIBuffer_PointInstance_Extend(const CVIBuffer_PointInstance_Extend & rhs)
	: CVIBuffer(rhs)
...
HRESULT CVIBuffer_PointInstance_Extend::NativeConstruct_Prototype(const _tchar* pShaderFilePath, const char* pTechniqueName, _uint iNumInstance)
{
	/* For. VertexBuffer */
	...
	m_pInstanceVtx = new VTXMATRIX_EXTEND[m_iNumInstance];
	...
	D3D11_INPUT_ELEMENT_DESC			ElementDesc[] = {
		...
		{ "TEXCOORD", 0, DXGI_FORMAT_R32_FLOAT , 1, 60, D3D11_INPUT_PER_INSTANCE_DATA, 1}
	};

	if (FAILED(CVIBuffer::SetUp_InputLayOuts(ElementDesc, 7, pShaderFilePath, pTechniqueName)))
		return E_FAIL;

	return S_OK;
}

HRESULT CVIBuffer_PointInstance_Extend::NativeConstruct(void * pArg)
{
...
HRESULT CVIBuffer_PointInstance_Extend::Render(_uint iPassIndex, _int iDrawCount, _int iStartIndex)
{
	...
	const _uint	 iStrides[2] = { sizeof(VTXPOINT), sizeof(VTXMATRIX_EXTEND) };
...
void CVIBuffer_PointInstance_Extend::Update_Instance(_float TimeDelta)
{
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
		*((VTXMATRIX_EXTEND*)MappedSubResource.pData + i) = m_pInstanceVtx[i];

	m_pDevice_Context->Unmap(m_pVBInstance, 0);

}

HRESULT CVIBuffer_PointInstance_Extend::SetUp_VertexBuffer_Desc(D3D11_USAGE Usage, _uint BindFlag, _uint iCpuAccessFlag)
{
	ZeroMemory(&m_InstanceBufferDesc, sizeof(D3D11_BUFFER_DESC));

	m_InstanceBufferDesc.ByteWidth = sizeof(VTXMATRIX_EXTEND) * m_iNumInstance;
	...
	m_InstanceBufferDesc.StructureByteStride = sizeof(VTXMATRIX_EXTEND);

	return S_OK;
}

HRESULT CVIBuffer_PointInstance_Extend::SetUp_VertexSubResourceData(void * pSysMem)
{
...
CVIBuffer_PointInstance_Extend * CVIBuffer_PointInstance_Extend::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context, const _tchar* pShaderFilePath, const char* pTechniqueName, _uint iNumInstance)
{
	CVIBuffer_PointInstance_Extend*		pInstance = new CVIBuffer_PointInstance_Extend(pDevice, pDevice_Context);

	if (FAILED(pInstance->NativeConstruct_Prototype(pShaderFilePath, pTechniqueName, iNumInstance)))
	{
		MSG_BOX("Failed to Creating Instance (CVIBuffer_PointInstance_Extend) ");
...
CComponent * CVIBuffer_PointInstance_Extend::Clone(void * pArg)
{
	CVIBuffer_PointInstance_Extend*		pInstance = new CVIBuffer_PointInstance_Extend(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned Instance (CVIBuffer_PointInstance_Extend) ");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CVIBuffer_PointInstance_Extend::Free()
{
	CVIBuffer::Free();

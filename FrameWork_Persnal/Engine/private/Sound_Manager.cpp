...
void CSound_Manager::UpdateSound(void)
{
	FMOD_System_Update(m_pSystem);
}

void CSound_Manager::StopSound(CHANNEL_TYPE eChannel)
{
...
void CSound_Manager::Play_Sound(TCHAR * pSoundKey, CHANNEL_TYPE eChannel, _fvector vecCamEye, _fvector vecSoundPos, _float fRange, _int iSoundNum, _bool bPlayOnce)
{
	...
	{
		if (CSound_Manager::GetInstance()->IsPlaying(eChannel))
			return;
	}

	_float fDistance = XMVectorGetX(XMVector3Length(vecCamEye - vecSoundPos));
	if (fDistance > fRange)
		return;

//#include "Shader_Bloom.hlsl"

texture2D		g_DiffuseTexture;

sampler DiffuseSampler = sampler_state
{
	AddressU = wrap;
	AddressV = wrap;
};

texture2D		g_ShadeTexture;

sampler ShadeSampler = sampler_state
{
	AddressU = wrap;
	AddressV = wrap;
};

texture2D		g_SpecularTexture;

sampler SpecularSampler = sampler_state
{
	AddressU = wrap;
	AddressV = wrap;
};


struct VS_IN
{
	float4 vPosition : POSITION;	
	float2 vTexUV : TEXCOORD;
	
};

struct VS_OUT
{
	float4 vPosition : SV_POSITION;	
	float2 vTexUV : TEXCOORD;
};


VS_OUT VS_MAIN(VS_IN In)
{
	VS_OUT			Out = (VS_OUT)0;
	
	Out.vPosition = In.vPosition;
	Out.vTexUV = In.vTexUV;

	return Out;
}


float3 ToneMapACES(float3 hdr)
{
	const float A = 2.51, B = 0.03, C = 2.43, D = 0.59, E = 0.14;
	return saturate((hdr * (A * hdr + B)) / (hdr * (C * hdr * D) + E));
}


struct PS_IN
{
	float4 vPosition : SV_POSITION;
	float2 vTexUV : TEXCOORD;
};

struct PS_OUT
{
	vector	vColor : SV_TARGET0;
};

PS_OUT PS_MAIN(PS_IN In)
{
	PS_OUT		Out = (PS_OUT)0;

	vector	vDiffuseDesc = g_DiffuseTexture.Sample(DiffuseSampler, In.vTexUV);
	vector	vShadeDesc = g_ShadeTexture.Sample(ShadeSampler, In.vTexUV);
	vector	vSpecularDesc = g_SpecularTexture.Sample(SpecularSampler, In.vTexUV);

	//if(vDiffuseDesc.a == 1.f)
	//	Out.vColor = vDiffuseDesc;
	
	Out.vColor = vDiffuseDesc * vShadeDesc + vSpecularDesc;

	//float3 vColor = Out.vColor.rgb;
	//Out.vColor = vector(ToneMapACES(vColor), 1.f);
	//PS_BLOOM

	if (Out.vColor.a < 0.02f)
		discard;

	return Out;
}

PS_OUT PS_DIFFUSE_BLENDER(PS_IN In)
{
	PS_OUT		Out = (PS_OUT)0;

	vector	vDiffuseDesc = g_DiffuseTexture.Sample(DiffuseSampler, In.vTexUV); // 기존에 있던 대상
	vector	vShadeDesc = g_ShadeTexture.Sample(ShadeSampler, In.vTexUV); // 새로 그려질 대상 


	//Out.vColor = vDiffuseDesc;
	//if (vShadeDesc.a > 0)
	//{
	//	Out.vColor = vShadeDesc;
	//}

	Out.vColor = vDiffuseDesc;
	if (vShadeDesc.a > 0)
	{
		Out.vColor.rgb = vShadeDesc.rgb;
	}

	return Out;
}

RasterizerState Rasterizer_Solid
{
	FillMode = solid;
	CullMode = back;
	FrontCounterClockwise = false;
};

DepthStencilState DepthStecil_ZEnable
{
	DepthEnable = false;	
};

BlendState BlendState_None
{
	BlendEnable[0] = false;
};

technique11		DefaultTechnique
{
	pass Directional
	{		
		SetRasterizerState(Rasterizer_Solid);
		SetDepthStencilState(DepthStecil_ZEnable, 0);
		SetBlendState(BlendState_None, vector(0.f, 0.f, 0.f, 0.f), 0xffffffff);

		VertexShader = compile vs_5_0 VS_MAIN();
		GeometryShader = NULL;
		PixelShader = compile ps_5_0 PS_MAIN();
	}

	pass DiffuseBlender
	{
		SetRasterizerState(Rasterizer_Solid);
		SetDepthStencilState(DepthStecil_ZEnable, 0);
		SetBlendState(BlendState_None, vector(0.f, 0.f, 0.f, 0.f), 0xffffffff);

		VertexShader = compile vs_5_0 VS_MAIN();
		GeometryShader = NULL;
		PixelShader = compile ps_5_0 PS_MAIN();
	}
};



